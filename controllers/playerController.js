...
const generateTeams = async (req, res) => {
	try {
		const numberOfTeams = parseInt(req.params.numberOfTeams, 10);
		...
		players.forEach((player) => {
			...
			const weightedAverage = skills.reduce(
				(acc, skill) => acc + player[skill] * weights[skill],
				0
			);
		...
		const weightedAveragesSorted = weightedAveragesArray.sort(
			(a, b) => b[1] - a[1]
		);
		...
		teamsForAverageTeamPower.forEach((team, index) =>
			averageTeamPower.push(
				`Team ${index + 1}: ${
					team.reduce((acc, player) => acc + player[1], 0) / team.length
				}`
			)
		);
...
const getAveragePower = async (req, res) => {
	try {
		const id = req.params.id;
		const user = req.user;
		const playerObject = await Player.findOne({ _id: id, owner: user._id });
		...
		const totalPower = skills.reduce(
			(acc, skill) => acc + (player[skill] || 0),
			0
		);
		const averagePower = totalPower / skills.length;

		res.send({ averagePower });
	} catch (error) {
		res.status(400).send({ error: error.message });
		console.error(error);
	}
